...
const schema = z.object({
    ...
    username: z.string().min(1, { message: "Username is required" }).transform((val) => val.trim()),
    password: z.string().min(6, { message: "Password must be at least 6 characters" }).transform((val) => val.trim()),
  });
    ...
    try {
        ...
        const hashedPassword = bcrypt.hashSync(reqData.password, 10);
            try {
              ...
              throw new Error('could create and save user to db for some reaosn: ', err);
            }
    }
    catch(e) {
