...
export async function POST(req) {
  ...
  console.log(
    ...
    if (user) {
        console.log('already have an acc with this google acc, returning existing user obj')
    ...
    try {
        console.log("this is a new acc. trying to create and save google user to db now");
        ...
        return NextResponse.json({ message: "Logged in", user, token }, { status: 201 });
      } catch (err) {
        console.log('errored: ', err);
        throw new Error(
          "couldnt create and save user to db for some reaosn");
}}
